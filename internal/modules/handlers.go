...
func plainTextCommandMiddleware(next telegram.MessageHandler) telegram.MessageHandler {
	return func(m *telegram.NewMessage) error {
		if !m.IsService() && m.Message != nil && m.Message.Message != "" && !strings.HasPrefix(m.Message.Message, "/") {
			for _, h := range handlers {
				re := regexp.MustCompile("^" + h.Pattern + "$")
				if re.MatchString(m.Message.Message) {
					newText := "/" + m.Message.Message
					m.Message.Message = newText
					m.Message.Entities = append(m.Message.Entities, &telegram.MessageEntityBotCommand{
						Offset: 0,
						Length: int32(len(newText)),
					})
					break
				}
			}
